...
namespace NotificationService.Messaging
{
    public class MessageSentConsumer : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("Waiting for MessageSent messages from queue: {QueueName}", QueueName);
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                try
                {
                    ...
                    var evt = JsonSerializer.Deserialize<MessageSentEvent>(message);
                ...
                {
                    _logger.LogError(ex, "Error processing MessageSent event");
