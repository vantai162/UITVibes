...
{
    public class PostMentionedConsumer : BackgroundService
    {
        private const string QueueName = "post.mentioned";
        private readonly ILogger<PostMentionedConsumer> _logger;
        private readonly IServiceProvider _serviceProvider;
        private IConnection? _connection;
        private IChannel? _channel;

        public PostMentionedConsumer(
            ILogger<PostMentionedConsumer> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("Waiting for PostMentioned messages from queue: {QueueName}", QueueName);
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                try
                {
                    ...
                    var evt = JsonSerializer.Deserialize<PostMentionedEvent>(message);
                ...
                {
                    _logger.LogError(ex, "Error processing PostMentioned event");
