...
namespace PostService.ServiceLayer.Implementation
{
    public class UserProfileRpcClient: IUserProfileRpcClient
    {
        ...
        public async Task<UserProfileRpcResponse?> GetProfileAsync(string displayname, CancellationToken cancellationToken = default)
        {
            await using var connection = await _connectionFactory.CreateConnectionAsync(cancellationToken);
            await using var channel = await connection.CreateChannelAsync(cancellationToken: cancellationToken);
            ...
            var request = new UserProfileRpcRequest { DisplayName = displayname };
            var body = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(request));
            ...
            if (completed != tcs.Task)
            {
                _logger.LogWarning("User profile RPC timeout for displayname {DisplayName}", displayname);
                return null;
            }
