...
namespace UserService.Messaging
{
    public class UserProfileRpcConsumer : BackgroundService
    {
        ...
        private readonly ILogger<UserProfileRpcConsumer> _logger;
        private readonly IServiceProvider _serviceProvider;
        private IConnection? _connection;
        private IChannel? _channel;

        public UserProfileRpcConsumer(
            ILogger<UserProfileRpcConsumer> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                var response = new UserProfileRpcResponse { Found = false };

                try
                {
                    var payload = Encoding.UTF8.GetString(ea.Body.ToArray());
                    var request = JsonSerializer.Deserialize<UserProfileRpcRequest>(payload);
                    ...
                    if (request != null)
                    {
                        ...
                        if (profile != null)
                        {
                            response = new UserProfileRpcResponse
                            {
                ...
                {
                    _logger.LogError(ex, "Error processing UserProfile RPC request");
                }
